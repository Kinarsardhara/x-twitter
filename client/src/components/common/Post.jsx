...
import { apiUrl, formatPostDate } from "../../utils/date";
...
const Post = ({ post }) => {
  ...
  const isBookMark = post.bookMarks.includes(authUser.user._id);
  if (!post.user) {
    return null;
  }
  ...
  const { mutate: bookMarkPost, isPending: isBookMarking } = useMutation({
    mutationFn: async () => {
      try {
        const res = await fetch(`${apiUrl}api/posts/bookmark/${post._id}`, {
          method: "POST",
          credentials: "include"
        });
        const data = await res.json();
    ...
    onSuccess: (updatedBookMark) => {
      queryClient.setQueryData(["posts"], (oldData) => {
        return oldData.map((p) => {
          if (p._id === post._id) {
            return { ...p, bookMarks: updatedBookMark };
          }
          return p;
        });
      });
